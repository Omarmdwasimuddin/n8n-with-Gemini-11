## n8n-with-Gemini-11

#### Click: Add first step--->search & click: webhook--->HTTP Method POST daw & POST url copy koro--->index.html e scroll kore nicher dike WEBHOOK_URL e POST url paste koro
![](https://imgur.com/1m0j0Jc.png)

#### close koro n8n powershell (Ctrl+c)--->prompt daw: $env:N8N_CORS_ALLOWED_ORIGINS="*"; n8n --->Enter daw
#### click koro Webhook er + sign--->search & click: Google Gemini--->click: Message a model--->click: Set up credential--->notun ekta browser open koro--->search: ai studio gemini--->
![](https://imgur.com/1m0j0Jc.png)
